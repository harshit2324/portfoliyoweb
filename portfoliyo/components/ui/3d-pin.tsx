...
// Separate client-only animation wrapper
  ...
  return (
    <div className="pointer-events-none w-full h-80 flex items-center justify-center opacity-0 group-hover/pin:opacity-100 z-[60] transition duration-500">
      <div className="w-full h-full -mt-7 flex-none inset-0">
        ...
        >
          {[0, 2, 4].map((delay) => (
            <AnimatedCircle key={delay} delay={delay} />
          ))}
        </div>

        <div className="absolute right-1/2 bottom-1/2 translate-y-[14px]">
          <div className="bg-gradient-to-b from-transparent to-cyan-500 w-px h-20 group-hover/pin:h-40 blur-[2px]" />
          <div className="bg-gradient-to-b from-transparent to-cyan-500 w-px h-20 group-hover/pin:h-40" />
          <div className="translate-x-[1.5px] bg-cyan-600 w-[4px] h-[4px] rounded-full z-40 blur-[3px]" />
          <div className="translate-x-[0.5px] bg-cyan-300 w-[2px] h-[2px] rounded-full z-40" />
        </div>
      </div>
    </div>
  );
